...
#print(paste("Current Working Directory:", currWorkingDir))
...
for(i in 1:length(experiment_list)) {
  f = paste(currWorkingDir, "/experiments/", experiment_list[i], "/results/postPrediction/input_summary.csv", sep="")
...
for(i in 1:length(experiment_list)) {
  for (j in 1:length(tool_list)) {
    if(names(tool_list)[j] == "simulation") {
      next
    }  else {
...
# fix issue if rel_abun is not in the range [0,100] but [0,1]
# data.tool.long %>% group_by(tool_name) %>% summarize(max = max(rel_abun))
if(data.tool.long %>% filter(tool_name != "simulation") %>% pull(rel_abun) %>% max() <= 1 & data.tool.long %>% filter(tool_name == "simulation") %>% pull(rel_abun) %>% max() > 1){
  data.tool.long <- data.tool.long %>% mutate(rel_abun = ifelse(tool_name == "simulation", rel_abun/100, rel_abun))
}
...
for(tc in tabs){
  if( tc == "Overall"){
    ...
    if(identical(tabs.dict[[tc]], character(0))){
      tabs.dict[[tc]] <- tc
    }
  }
}
...
abundanceEstimationPlot <- function(data.error, data.long, title){
  ...
  ggplot(filter(data.plot, tool_name != "simulation"), aes(x=rel_abun_sim, y=rel_abun)) +
    ...
    facet_grid(tool_name ~ timecourse) +
    labs(title = "tool") +
...
errorEstimationPlot <- function(data.error, title){
  ...
  data.plot <- data.error %>%
    ...
    p <- ggplot(data.plot, aes(x=rel_abun_sim, y=Error, fill=tool_name, color=tool_name)) +
